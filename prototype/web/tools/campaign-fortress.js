// fortress — generated by scratchpad gen.js (playtest strategy)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|DIRECTIVE|COUNTER|LAPSED/i.test(j.h)`;
exports.play = async (api) => {
  ...
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[];
    ...
    if(v.flagship>0 && arm("Polar Destabilization","Arctic Shelf")) did.push("Polar (earmark cash)");
    if(home().anomaly<-0.4 && arm("Cloud Seeding",HOME)) did.push("seed home");
    while(purse()>=18+10 && adapt<14){ if(!arm("Adaptation Investment",HOME)) break; adapt++; did.push("adapt#"+adapt); }
    if(sinceOp()>=3 && purse()>=6 && arm("Cloud Seeding",driest().name)) did.push("keep-alive seed");
    await step(did); }
  return log;
};
