// arson — generated by scratchpad gen.js (playtest strategy)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|DIRECTIVE|INTERCEPT|schoolteacher/i.test(j.h)`;
exports.play = async (api) => {
  ...
  const W={"South Asia":1.43,"Black Sea Steppe":1.16,"La Plata Basin":0.99,"Southeast Asia":0.87,"North China Plain":0.65,"Eastern Australia":0.6,"Yangtze Basin":0.59,"Sahel":0.55,"Horn of Africa":0.55,"Northern European Plain":0.48,"Japan (Kanto–Kansai)":0.48,"California Central Valley":0.4,"Canadian Prairies":0.4,"Cerrado":0.4};
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[];
    cont = v.rung>=4? 14 : v.rung>=3? 8 : 0;
    const dd=directive(); if(dd) did.push(dd);
    if(v.flagship>0 && arm("Polar Destabilization","Arctic Shelf")) did.push("Polar (earmark)");
    if(home().anomaly<-0.5 && arm("Cloud Seeding",HOME)) did.push("seed home");
    const dry=v.regions.filter(r=>W[r.name]&&r.anomaly<-0.5&&recent(r.name,6)===0).sort((a,b)=>W[b.name]-W[a.name]);
    for(const r of dry){ if(purse()<9+10) break; if(arm("Fire Enablement",r.name)) did.push("fire "+r.name+"("+r.anomaly+")"); }
    await step(did); }
  return log;
};
