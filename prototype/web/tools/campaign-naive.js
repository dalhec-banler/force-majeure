...
exports.play = async (api) => {
  const log=[];
  const driest=(v)=>v.regions.filter(r=>r.kind==="crop").sort((a,b)=>a.anomaly-b.anomaly)[0];
  let containment=0;
  for(let s=1;s<=40;s++){
    const v=api.view();
    if(!v.running) break;
    ...
    else if(d==="Answer them" && s>=24 && v.funds>50){ api.arm("Watershed Interference","Black Sea Steppe"); did.push("D7: answering them"); }
    else {
