// minimal — generated by scratchpad gen.js (playtest strategy)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|DIRECTIVE/i.test(j.h)`;
exports.play = async (api) => {
  ...
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[]; let n=0;
    cont = v.rung>=3? 8 : 0;
    if(v.flagship>0 && arm("Polar Destabilization","Arctic Shelf")){ did.push("Polar (earmark)"); n++; }
    if(home().anomaly<-0.45 && arm("Cloud Seeding",HOME)){ did.push("seed home"); n++; }
    if(n<2){ const r=driest(EXPORTERS); if(r.anomaly<-0.5 && purse()>=9+8 && arm("Fire Enablement",r.name)){ did.push("fire "+r.name); n++; }
      else if(s%2===0 && purse()>=14+8 && arm("Watershed Interference",recent(BSS,8)?"Eastern Australia":BSS)){ did.push("watershed"); n++; } }
    await step(did); }
  return log;
};
