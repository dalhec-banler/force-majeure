// hush — generated by scratchpad gen.js (playtest strategy)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|DIRECTIVE|INTERCEPT|investigation/i.test(j.h)`;
exports.play = async (api) => {
  ...
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[];
    ...
    if(purse()>=32+20 && arm("Stratospheric Aerosol Inj.")) did.push("SAI");
    const r=driest(EXPORTERS); if(purse()>=14+16){ const c=r.anomaly<-0.5?"Fire Enablement":"Watershed Interference"; if(arm(c,r.name)) did.push(c+" "+r.name); }
    cont = Math.floor(purse()*0.4); did.push("cont "+cont);
    await step(did); }
  return log;
};
