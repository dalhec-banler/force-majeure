// veil — generated by scratchpad gen.js (playtest strategy)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|INTERCEPT|investigation|EXPOSED|UNITED/i.test(j.h)`;
exports.play = async (api) => {
  ...
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[]; cont=0;
    if(v.flagship>0 && arm("ENSO Forcing")) did.push("ENSO (earmark)");
    let n=0; while(purse()>=32 && arm("Stratospheric Aerosol Inj.")) n++; if(n) did.push("SAI x"+n);
    if(!n && purse()>=9){ const r=driest(EXPORTERS); if(arm(r.anomaly<-0.5?"Fire Enablement":"Watershed Interference",r.name)) did.push("cheap strike "+r.name); }
    await step(did); }
  return log;
};
