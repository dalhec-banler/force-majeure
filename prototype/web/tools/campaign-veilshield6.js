// veilshield6 — veilshield family: adaptation shield + stacked aerosol waves (gen2.js)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|DIRECTIVE|INTERCEPT|investigation|WINDFALL|COUNTER/i.test(j.h)`;
exports.play = async (api) => {
  ...
  const C={"waves":[9,18,27],"maxSAI":6,"adapt":9,"keep":4,"saveWindow":3,"saveAbove":200,"flag":["Polar Destabilization","Polar Destabilization"],"cont":0,"selfburn":true};
  let adapt=0, waves=0;
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[];
    cont = C.cont? (v.rung>=5? 20 : v.rung>=4? C.cont : 0) : 0;
    const dd=directive({driver:"Stratospheric Aerosol Inj."}); if(dd) did.push(dd);
    if(v.flagship>0 && C.flag[waves<2?0:1] && arm(C.flag[waves<2?0:1], "Arctic Shelf")) did.push("earmark "+C.flag[waves<2?0:1]);
    const waveNow=C.waves.includes(s);
    if(waveNow){ let n=0; while(n<C.maxSAI && purse()>=32+C.keep && arm("Stratospheric Aerosol Inj.")) n++; if(n){ did.push("SAI x"+n); waves++; } }
    else {
      // shield: adaptation while cash allows, but keep a war chest before a wave
      const next=C.waves.find(w=>w>s), toWave=next? next-s : 99;
      const canShield = adapt<C.adapt && (toWave>C.saveWindow || v.funds>C.saveAbove);
      while(canShield && adapt<C.adapt && purse()>=18+C.keep){ if(!arm("Adaptation Investment",HOME)) break; adapt++; did.push("adapt#"+adapt); }
      // dry homeland with a >100 shield is a gift; otherwise defend it
      const h=home();
      if(adapt*12>100 && h.anomaly<-0.5 && C.selfburn && arm("Fire Enablement",HOME)) did.push("self-burn home (shield "+adapt*12+")");
      else if(adapt*12<80 && h.anomaly<-0.5 && arm("Cloud Seeding",HOME)) did.push("seed home");
