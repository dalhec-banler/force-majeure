// ensoflag — generated by scratchpad gen.js (playtest strategy)
exports.journalFilter = `j.c==="CHYRON"||/TREASURY|EARMARK|Pacific|DIRECTIVE|INTERCEPT/i.test(j.h)`;
exports.play = async (api) => {
  ...
  for(s=1;s<=40;s++){ v=api.view(); if(!v.running) break; const did=[];
    cont = v.rung>=4? 14 : v.rung>=3? 8 : v.rung>=2? 4 : 0;
    const dd=directive({driver:"Ocean Thermal Forcing"}); if(dd) did.push(dd);
    if(v.flagship>0 && arm("ENSO Forcing")) did.push("ENSO Forcing (earmark)");
    if(home().anomaly<-0.5 && arm("Cloud Seeding",HOME)) did.push("seed home");
    if(adapt<3 && s<=10 && purse()>=30 && arm("Adaptation Investment",HOME)){ adapt++; did.push("adapt#"+adapt); }
    const tgt=[BSS,"Eastern Australia","South Asia"].map(reg).sort((a,b)=>recent(a.name,8)-recent(b.name,8))[0];
    if(purse()>=30 && recent(tgt.name,8)===0){ const c=tgt.anomaly<-0.5?"Fire Enablement":"Watershed Interference"; if(arm(c,tgt.name)) did.push(c+" "+tgt.name); }
    await step(did); }
  return log;
};
