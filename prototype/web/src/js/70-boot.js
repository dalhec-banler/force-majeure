...
/* Briefing intent: a prospective director chooses a country and understands
   ...
   authored strengths/weaknesses explain the numbers. No metric-only cards. */
const CRISIS_START_BRIEFS={
 "North American Plains":{plus:["the largest harvest — export price rises reach your whole crop","steady weather and the lowest drought sensitivity among the big exporters"],minus:["repeated homeland interventions attract the rival's attention","protecting a large harvest also lowers the world price you sell into"]},
 "Black Sea Steppe":{plus:["a committee that funds fear generously (+4 mandate)","a major wheat exporter — scarcity abroad can pay at home"],minus:["weather swings wider and drought bites harder than in the Plains","the American programme can reach your watershed too"]},
 "North China Plain":{plus:["more of the harvest feeds home — less exposure to world-price swings","the Pacific's connection to your plain is known from the start"],minus:["little grain is exported, so foreign shortages bring smaller windfalls","a demanding climate and a parsimonious committee (−2 mandate)"]},
 "Northern European Plain":{plus:["the most drought-resistant harvest of the five programmes","your Atlantic connection is known — ocean operations can reach home"],minus:["a smaller harvest means less revenue from a price rise","a parsimonious committee (−2 mandate) makes unused wings expensive"]},
 "South Asia":{plus:["a large rice and wheat harvest — recovery matters to the whole market","the steadiest underlying weather of the five programmes"],minus:["drought bites hardest; a failed monsoon can undo a good year","the least generous committee (−4 mandate), with limited export windfalls"]},
};
...
{ const home=REG.find(r=>r.homeland);
  ...
  for(const [region,original] of Object.entries(STARTS).sort((a,b)=>(a[1].rank||9)-(b[1].rank||9))){
    const st=CRISIS?{...original,treasury:120,...CRISIS_START_BRIEFS[region]}:original;
    const b=document.createElement("button"); b.className="hp"+(region===HOMELAND?" on":"");
    b.setAttribute("aria-pressed",String(region===HOMELAND));
...
function showIntro(instant){                // fade the brief up
  ...
  i.style.display="flex"; $("briefsheet").scrollTop=0;
...
$("campaignmode").addEventListener('click',()=>{try{localStorage.setItem('fm.mode',CRISIS?'long':'crisis');sessionStorage.setItem(SKIP,"1");}catch(e){} location.reload();});
$("campaignlabel").textContent=CRISIS?"CAMPAIGN · SIXTEEN-REVIEW CRISIS · 2030–33":"CAMPAIGN · THE LONG RECORD · 1946–2060";
