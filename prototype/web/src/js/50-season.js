...
function historyBeats(row){
  ...
  if(canon){
    ...
    for(const e of HISTORY.eruptions) if(e.t===t){ cumDead+=e.toll||0;
      if((e.scale||1)>=1.2 || (e.toll||0)>=500 || (e.climate||0)>0) news(e.dl, e.line, (e.scale||1)>=1.3||(e.toll||0)>=2000); }
    for(const q of HISTORY.quakes) if(q.t===t){ cumDead+=q.toll||0;
      const famous=q.mag>=7.8 || (q.toll||0)>=5000;
      if(famous) news(q.dl, q.line, q.mag>=8.5||(q.toll||0)>=20000);
      if(q.mag>=8 && (q.toll||0)>=1000){ alertStrip(`EARTHQUAKE M${q.mag.toFixed(1)} — ${q.name.toUpperCase()}`); shakeNow(); }
      else if(q.mag>=8.5) shakeNow();
    }
  } else {
  ...
  for(const basin of ["WP","EP","NI","SI","SP"]){
    ...
    const show=bs.filter(s=>s.name && s.landfall && s.cat>=4 && !([...told].some(l=>new RegExp("\\b"+word.toLowerCase()+" "+s.name.toLowerCase()+"\\b").test(l)))).slice(0, 1);
    for(const s of show){
